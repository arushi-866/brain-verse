...
// Function to generate mind map data using Gemini with retry logic
async function generateMindMapWithGemini(text, retryCount = 0) {
  const maxRetries = 3;
  const baseDelay = 2000; // 2 seconds base delay

  try {
    console.log(`Sending prompt to Gemini... (attempt ${retryCount + 1}/${maxRetries + 1})`);
...
- Optional: Use "relatedTo" to show cross-links.
    ...
    // Check if it's a quota exceeded error (429 status or quota mentioned in message)
    if ((error.status === 429 || (error.message && error.message.includes('429'))) &&
      ...
      if (retryCount < maxRetries) {
        ...
        return generateMindMapWithGemini(text, retryCount + 1);
      } else {
        console.error('Max retries exceeded for Gemini API quota limit');
        throw new Error('Gemini API quota exceeded. Please try again later or consider upgrading your Google AI Studio plan.');
      }
    }
...
async function generateSummaryWithGemini(text, retryCount = 0) {
  const maxRetries = 3;
  ...
  try {
    ...
    if ((error.status === 429 || (error.message && error.message.includes('429'))) &&
      ...
      if (retryCount < maxRetries) {
        ...
        console.log(`Quota exceeded. Retrying summary in ${delay}ms... (attempt ${retryCount + 1}/${maxRetries})`);
        await new Promise(resolve => setTimeout(resolve, delay));
        return generateSummaryWithGemini(text, retryCount + 1);
      } else {
        throw new Error('Gemini API quota exceeded. Please try again later or consider upgrading your Google AI Studio plan.');
      }
    }
...
// Helper function for flashcard generation with retry logic
async function generateFlashcardsWithGemini(text, retryCount = 0) {
  const maxRetries = 3;
  const baseDelay = 2000;

  try {
    console.log(`Generating flashcards... (attempt ${retryCount + 1}/${maxRetries + 1})`);
    ...
    if ((error.status === 429 || (error.message && error.message.includes('429'))) &&
      ...
      if (retryCount < maxRetries) {
        const delay = baseDelay * Math.pow(2, retryCount);
        console.log(`Quota exceeded. Retrying flashcards in ${delay}ms... (attempt ${retryCount + 1}/${maxRetries})`);
        await new Promise(resolve => setTimeout(resolve, delay));
        return generateFlashcardsWithGemini(text, retryCount + 1);
      } else {
        throw new Error('Gemini API quota exceeded. Please try again later or consider upgrading your Google AI Studio plan.');
      }
    }
...
// Helper function for quiz generation with retry logic
async function generateQuizWithGemini(text, retryCount = 0) {
  const maxRetries = 3;
  const baseDelay = 2000;

  try {
    console.log(`Generating quiz... (attempt ${retryCount + 1}/${maxRetries + 1})`);
    ...
    if ((error.status === 429 || (error.message && error.message.includes('429'))) &&
      ...
      if (retryCount < maxRetries) {
        const delay = baseDelay * Math.pow(2, retryCount);
        console.log(`Quota exceeded. Retrying quiz in ${delay}ms... (attempt ${retryCount + 1}/${maxRetries})`);
        await new Promise(resolve => setTimeout(resolve, delay));
        return generateQuizWithGemini(text, retryCount + 1);
      } else {
        throw new Error('Gemini API quota exceeded. Please try again later or consider upgrading your Google AI Studio plan.');
      }
    }
  ...
  try {
    ...
    if (!text || text.trim() === '') {
      return res.status(400).json({
        ...
        message: 'Please provide text to generate a mind map'
      });
    }
    ...
    if (error.message && error.message.includes('quota exceeded')) {
      return res.status(429).json({
        ...
        message: 'Gemini API free tier limit reached. Please try again later or upgrade your Google AI Studio plan.',
...
app.post('/api/process-document', upload.single('file'), async (req, res) => {
  try {
    ...
    if (error.message.includes('Only PDF and text files')) {
      ...
      statusCode = 429;
      errorMessage = 'Gemini API free tier limit reached. Please try again later or upgrade your Google AI Studio plan.';
    }
...
app.post('/generate-quiz', async (req, res) => {
  try {
    ...
    const quizQuestions = await generateQuizWithGemini(text);
